...
try:
  ...
  print '#' * 79
  print 'paramiko: http://www.lag.net/paramiko/'
  print 'pycrypto: http://www.voidspace.org.uk/python/modules.shtml#pycrypto'
  print 'pywin32: http://sourceforge.net/projects/pywin32/files/pywin32'
  print '#' * 79
  raise
...
remote_root_http = 'http://rd.gnus.org/upload/'

# The drop folder as seen from the user account.
remote_root_path = '/var/www/rd_gnus_org/upload/'
...
def set_clipboard_paths(files):
  html_paths = [remote_root_http + urllib.quote(f) for f in files]
  text = '\r\n'.join(html_paths) + '\r\n'
